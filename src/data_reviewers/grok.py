...
Grok-based evaluator for one structured JSON data row.

This version uses a stricter JSON schema and stronger Python-side validation so
the model output is closer to OpenAI-style structured extraction.
...
def get_response_schema(row: Dict[str, Any]) -> Dict[str, Any]:
    """Return the JSON schema for Grok structured output."""
...
def validate_output(
    ...
    """Validate Grok structured output."""
    required_keys = [
        "id",
        "rating",
        "confidence",
    ...
    if not isinstance(result, dict):
        raise ValueError("Grok output is not a dict.")

    extra_keys = set(result.keys()) - set(required_keys)
    if extra_keys:
        raise ValueError(f"Grok output contains unexpected keys: {sorted(extra_keys)}")
    ...
    if result["id"] != expected_id:
        raise ValueError(
            f"id must equal the input row id. Expected {expected_id!r}, got {result['id']!r}."
        )
...
def make_fallback_result(
    ...
    """Return a fallback result when Grok fails after all retries."""
    return {
        "id": row_id,
        "review_status": "error",
        "rating": 1,
        "confidence": 1,
        "concerns": [
            f"Grok evaluation failed due to API, parsing, or output validation error: {error_message}"
        ],
        "solutions": [
            "Retry this row manually or inspect the prompt, schema, SDK version, and row content."
        ],
        "resolved_data_row": {},
    }
...
def extract_response_text(response: Any) -> str:
    """
    Extract text from an xAI/OpenAI-compatible Responses API object.

    This helper is intentionally defensive because SDK response objects may
    ...
    raise ValueError("Could not extract text from Grok response.")


def evaluate_with_grok(
    row: Dict[str, Any],
    row_id: str,
    system_prompt_path: str,
    model: str = "grok-4.20-beta-latest-non-reasoning",
    ...
    """
    Evaluate one row with Grok and return standardized structured output.
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if not text or not text.strip():
                raise ValueError("Grok returned empty response text.")
